...
function Stats(job, config) {
  ...
  const api = {
    ...
    pretty: (emoji = true) => {
      return (
        `${emoji ? "❓  " : ""}${job.name}\n` +
        `${emoji ? "✏️  " : ""}${job.buffer.length} cmds\n` +
        `${emoji ? "✏️  " : ""}${distance} mm\n` +
        `${emoji ? "⏳  " : ""}${duration.estimation.formatted} (${
          duration.min.formatted
        } ~ ${duration.max.formatted})`
      );
    },
  };
  ...
  function calcDuration(cmds) {
    ...
    return {
      ...
      min: {
        ...
        formatted: variation.min
          ? secondToTime(variation.min)
          : secondToTime(duration - duration * 0.1),
      },
      max: {
        value: variation.max ? variation.max : duration + duration * 0.1,
        formatted: variation.max
          ? secondToTime(variation.max)
          : secondToTime(duration + duration * 0.1),
      },
    };
  }
